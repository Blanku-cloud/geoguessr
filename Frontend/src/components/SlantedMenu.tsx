...
export default function SlantedMenu() {
  const mode = gameMode.map((obj) => (
    <Link
      to="/"
      className="hover:text-indigo-300 hover:scale-105 h-20 flex items-center transition duration-75 ease-in-out"
    >
      {obj.mode}
    </Link>
  ));
  const mis = miscillieous.map((obj) => (
    <Link
      to="/"
      className="transition duration-75 ease-in-out hover:text-indigo-300 hover:scale-105 w-fit"
    >
      {obj.item}
    </Link>
  ));
  const icon = icons.map((obj) => (
    <Link to="/" className="flex flex-col items-center hover:scale-105">
      {obj.icon}
      {obj.place}
    </Link>
  ));
